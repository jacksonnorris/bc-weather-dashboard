...
var city = 'Charlotte';
var queryURL = `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=imperial&type=accurate&APPID=${apiKey}`;
var dayArray;
...
const getWeatherInfo = async (newCity) => {
    try {
        ...
        city = newCity;
        queryURL = `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=imperial&type=accurate&APPID=${apiKey}`;
        ...
        const ocURL = await request(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=imperial&type=accurate&APPID=${apiKey}`);
